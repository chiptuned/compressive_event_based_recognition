...
%LEVELCROSSING Summary of this function goes here
...
for ind = 1:numel(data)
    ...
    if (cur_level > last_level) % On ev
        for (level = last_level+1:1:cur_level)
            ts_rect = round(old_ts+(level-old_loc)*(time-old_ts)/(loc-old_loc)); % round?
            ts = [ts;ts_rect];
            val = [val;level];
            p = [p;1];
            last_level = level;
    ...
    elseif (cur_level < last_level) % Off ev
        for (level = last_level-1:-1:cur_level+1)
            ts_rect = round(old_ts+(level-old_loc)*(time-old_ts)/(loc-old_loc));
            ts = [ts;ts_rect];
            val = [val;level];
            p = [p;0];
            last_level = level;
